...
jaspar <- jcounts
inset <- c(sf, jaspar)
jtot <- 1554 # total number of JASPAR dyads used in calculations
sftot <- 1466 # total number of SF dyads used in calculations
...
jdf <- data.frame(x, jsd$V1[-30], type=as.factor("JASPAR MA0363.2"))
...
plot <- ggplot(all_df, aes(distance, count, Site_type, colour=Site_type))+
	ggtitle("  Consensus sequence dyads located in merged small fragment \n peaks identified by MACS3 from Reb1 SpLiT-ChEC-seq coverage")+
...
plot + scale_fill_discrete(name='Consensus sequence source', labels=c('Reb1 SpLiT-ChEC seq small fragment peaks', 'Reb1 JASPAR entry MA0363.2'))
