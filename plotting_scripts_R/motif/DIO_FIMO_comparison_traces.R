...
sdf <- data.frame(x, sfd$V1[-30], type=as.factor("Minimal motif"))
...
plot <- ggplot(all_df, aes(distance, count, Site_type, colour=Site_type))+
	...
	ylab("Proportion of sites in regions")+
	xlab("Merged region size (bp)")+
	theme_bw()+
	scale_colour_manual(values = c('black', 'blue'))+
	scale_fill_discrete(name='Consensus sequence source', labels=c('Abf1 SpLiT-ChEC seq small fragment peaks', 'Abf1 JASPAR entry MA0265.2'))
show(plot)
